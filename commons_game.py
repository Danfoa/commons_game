...
if __name__ == "__main__":
    ...
    logdir = "logs/MAP=%s-AGENTS=%d-EP=%d" % (MAP_TYPE, N_AGENTS, EPISODES)
    social_metrics_writer = tf.summary.create_file_writer(logdir + "/social_metrics")

    env = HarvestCommonsEnv(ascii_map=SMALL_HARVEST_MAP, num_agents=N_AGENTS, render=True,
                            agent_view_range=AGENT_VIEW_RANGE)
    obs = env.reset()
    ...
    for episode in range(EPISODES + 1):
            ...
            if t > START_LEARNING:
                ...
                if t % TARGET_UPDATE_ITERATION == 0:  # Update target model with learned changes
                    ...
                    with social_metrics_writer.as_default():
                        tf.summary.histogram('mse_Q', data=losses, step=t + (STEPS * episode))
        ...
        social_metrics = env.get_social_metrics(episode_steps=STEPS)
    ...
    plt.subplot(222)
    plt.plot(range(len(metrics[:, 1])), metrics[:, 1])
    plt.ylabel("Equality")
    plt.xlabel("Episode")
    plt.grid("on", alpha=0.15)
    plt.subplot(223)
    plt.plot(range(len(metrics[:, 2])), metrics[:, 2])
    plt.ylabel("Sustainability")
    plt.xlabel("Episode")
    plt.grid("on", alpha=0.15)
    plt.subplot(224)
    plt.plot(range(len(metrics[:, 3])), metrics[:, 3])
