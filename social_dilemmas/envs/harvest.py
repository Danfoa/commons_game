...
class HarvestCommonsEnv(MapEnv):
    ...
    def __init__(self, ascii_map=HARVEST_MAP, num_agents=1, render=False, agent_view_range=HARVEST_VIEW_SIZE):
        self.apple_points = []
    ...
    def setup_agents(self):
        ...
        for i in range(self.num_agents):
            ...
            agent = HarvestAgent(agent_id, spawn_point, rotation, grid, view_len=self.agent_view_range)
            # grid = util.return_view(map_with_agents, spawn_point,
            #                         HARVEST_DEFAULT_VIEW_SIZE, HARVEST_DEFAULT_VIEW_SIZE)
    ...
    def custom_map_update(self):
        ...
        for agent_id, agent in self.agents.items():
            if agent.remaining_timeout > 0:
                ...
                if not np.any(agent.pos == -1):
                    self.update_map([[agent.pos[0], agent.pos[1], '@']])
                    agent.pos = np.array([-1, -1])
            if agent.remaining_timeout == 0 and np.any(agent.pos == -1):
